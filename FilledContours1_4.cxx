...
// 原始的demo 加载json文件 然后导出成 geojson 格式的数据    新增输入的参数 colorScale的参数 用于设置颜色
...
int main(int argc, char* argv[])
{
  ...
  // Extract color scale data
  Json::Value& colorScale = colorScaleRoot["colorScale"];
  if (colorScale.isNull() || !colorScale.isArray()) {
    std::cerr << "Invalid color scale format. Expected 'colorScale' array." << std::endl;
    return EXIT_FAILURE;
  }
  ...
  try {
    ...
    // Use lineColor from colorScale for contour lines if available

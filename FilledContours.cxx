...
std::string ExportContoursToGeoJSON(vtkPolyData* contours, double zMin, double zMax, const std::string& filename = "") {
    ...
    std::string jsonStr = writer.write(geoJSON);
...
std::string ExportFilledContoursToGeoJSON(vtkPolyData* filledContours, double zMin, double zMax, const std::string& filename = "") {
...
// Core function that processes data into contours
int processContoursCore(
    Json::Value& data,
    Json::Value& colorScale,
    vtkPolyData*& smoothContoursOutput,
    vtkPolyData*& filledContoursOutput,
    double& zmin,
    double& zmax
) {
    try {
        ...
        if (data.isNull()) {
            std::cerr << "Error: No data found in JSON" << std::endl;
            return EXIT_FAILURE;
        }
        ...
        zmin = data.get("zmin", 0.0).asDouble();
        zmax = data.get("zmax", 1.0).asDouble();
        ...
        smoothContoursOutput = smoothContours->GetOutput();
        filledContoursOutput = filledContours->GetOutput();
        
        return EXIT_SUCCESS;
    }
    catch(const std::exception& e) {
        std::cerr << "Exception in processContoursCore: " << e.what() << std::endl;
        return EXIT_FAILURE;
    }
    catch(...) {
        std::cerr << "Unknown exception in processContoursCore" << std::endl;
        return EXIT_FAILURE;
    }
}

// Implementation of header-defined functions

int processContours(
    const std::string& dataJsonPath,
    const std::string& colorScalePath,
    const std::string& contourLinesOutputPath,
    const std::string& filledContoursOutputPath
) {
    try {
        // Read the JSON data file
        std::cout << "Opening data JSON file: " << dataJsonPath << std::endl;
        std::ifstream jsonFile(dataJsonPath);
        if (!jsonFile.is_open()) {
            std::cerr << "Failed to open JSON file: " << dataJsonPath << std::endl;
            return EXIT_FAILURE;
        }
        ...
        JSONCPP_STRING errs;
        if (!Json::parseFromStream(builder, jsonFile, &data, &errs)) {
        ...
        std::cout << "Opening color scale JSON file: " << colorScalePath << std::endl;
        std::ifstream colorScaleFile(colorScalePath);
        if (!colorScaleFile.is_open()) {
            std::cerr << "Failed to open color scale JSON file: " << colorScalePath << std::endl;
            return EXIT_FAILURE;
        }

        Json::Value colorScale;
        if (!Json::parseFromStream(builder, colorScaleFile, &colorScale, &errs)) {
            std::cerr << "Error parsing color scale JSON: " << errs << std::endl;
            return EXIT_FAILURE;
        }
        colorScaleFile.close();
        
        // Process the data
        vtkPolyData* smoothContoursOutput = nullptr;
        vtkPolyData* filledContoursOutput = nullptr;
        double zmin, zmax;
        
        int result = processContoursCore(data["data"], colorScale, smoothContoursOutput, filledContoursOutput, zmin, zmax);
        if (result != EXIT_SUCCESS || !smoothContoursOutput || !filledContoursOutput) {
            std::cerr << "Failed to process contours" << std::endl;
            return EXIT_FAILURE;
        }
        
        // Export the results to GeoJSON files
        ExportContoursToGeoJSON(smoothContoursOutput, zmin, zmax, contourLinesOutputPath);
        ExportFilledContoursToGeoJSON(filledContoursOutput, zmin, zmax, filledContoursOutputPath);
        
        return EXIT_SUCCESS;
    }
    catch (const std::exception& e) {
        std::cerr << "Exception in processContours: " << e.what() << std::endl;
        return EXIT_FAILURE;
    }
    catch (...) {
        std::cerr << "Unknown exception in processContours" << std::endl;
        return EXIT_FAILURE;
    }
}
...
int processContoursFromStrings(
    ...
    try {
        // Parse the JSON data from strings
        Json::Value data;
        Json::Value colorScale;
        Json::CharReaderBuilder builder;
        ...
        // Parse data JSON
        if (!reader->parse(dataJsonContent.c_str(), dataJsonContent.c_str() + dataJsonContent.length(), &data, &errs)) {
            std::cerr << "Error parsing data JSON: " << errs << std::endl;
            delete reader;
            return EXIT_FAILURE;
        }
        
        // Parse color scale JSON
        if (!reader->parse(colorScaleContent.c_str(), colorScaleContent.c_str() + colorScaleContent.length(), &colorScale, &errs)) {
            std::cerr << "Error parsing color scale JSON: " << errs << std::endl;
            delete reader;
            return EXIT_FAILURE;
        }
        ...
        std::cerr << "Exception in processContoursFromStrings: " << e.what() << std::endl;
        return EXIT_FAILURE;
    }
    catch (...) {
        std::cerr << "Unknown exception in processContoursFromStrings" << std::endl;
        return EXIT_FAILURE;
    }
}
...
int processContoursToStrings(
    ...
    try {
        ...
        // Export the results to strings
        contourLinesOutput = ExportContoursToGeoJSON(smoothContoursOutput, zmin, zmax);
        filledContoursOutput = ExportFilledContoursToGeoJSON(filledContoursOutput, zmin, zmax);
        
        return EXIT_SUCCESS;
    }
    catch (const std::exception& e) {
        std::cerr << "Exception in processContoursToStrings: " << e.what() << std::endl;
        return EXIT_FAILURE;
    }
    catch (...) {
        std::cerr << "Unknown exception in processContoursToStrings" << std::endl;
        return EXIT_FAILURE;
    }
}
...
#ifdef __EMSCRIPTEN__
EMSCRIPTEN_BINDINGS(FilledContours) {
    emscripten::function("processContours", &processContours);
    emscripten::function("processContoursFromStrings", &processContoursFromStrings);
    emscripten::function("processContoursToStrings", &processContoursToStrings);
}
#endif